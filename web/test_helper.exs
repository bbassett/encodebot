defmodule Test.Encodebot.Resource do
  ...
  defmacro __using__(_) do
    ...
    quote do
      ...
      defp prepare_request(conn, context) do
        {method, path_info} = @router.resolve(@subject, conn.private[:encodebot_test_params] || %{})

        request_path = "/" <> Enum.join(path_info, "/")
        body = conn.private[:encodebot_test_body]

        conn
        |> Plug.Adapters.Test.Conn.conn(method, request_path, body)
        |> Map.put(:remote_ip, conn.remote_ip || {127, 0, 0, 1})
        |> Map.put(:query_string, conn.private[:encodebot_test_query] || "")
      end
    end
  end
  ...
  defmacro affordance(body \\ [do: nil]) do
    quote do
      ...
      |> Plug.Conn.put_private(:mazurka_route, Test.Encodebot.Resource.AffordanceProxy)
      |> Plug.Conn.put_private(:mazurka_params, conn.private[:encodebot_test_params] || %{})
    end
  end

  defmacro params(params) do
    quote do
      var!(conn) = Plug.Conn.put_private(var!(conn), :encodebot_test_params, unquote(params))
    end
  end
    ...
    quote do
      var!(conn) = case unquote(content) do
        content when is_map(content) ->
          var!(conn) = Plug.Conn.put_private(var!(conn), :encodebot_test_body, Poison.encode!(content))
  ...
  defmacro query(content) do
    quote do
      var!(conn) = case unquote(content) do
        content when is_map(content) ->
          Plug.Conn.put_private(var!(conn), :encodebot_test_query, URI.encode_query(content))
      end
    end
  end
