defmodule EncodeBot.Protocol.HTTP.Router.Helper do
  defmacro __using__(_) do
    quote do
      use Plug.Builder
      use Concerto, [root: "#{System.cwd!}/web",
                     ext: ".ex",
                     module_prefix: EncodeBot.Resource]
      ...
      def match(%Plug.Conn{} = conn, _opts) do
        case match(conn.method, conn.path_info) do
          ...
          nil ->
            conn
            |> put_private(:mazurka_route, EncodeBot.Resource.Error.NotFound.GET)
            |> put_private(:mazurka_resource, EncodeBot.Resource.Error.NotFound.GET)
            |> put_private(:mazurka_resource_params, %{})
